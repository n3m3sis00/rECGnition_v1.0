...
hparams = {
    ...
    "channels" : 1,
    'is fine tune' : "no",
    "Description" : "With high batch size but same learning rate learning rate trained on tpu",
    "folds" : 9
}
...
if tpu:
    ...
    yapl.config.BATCH_SIZE = 64 * strategy.num_replicas_in_sync 
else:
    strategy = tf.distribute.get_strategy() 

#configuration

with open("/kaggle/input/mitbih-images-gaf-128x128/image_count.json") as f:
...
for fold in range(0, yapl.config.FOLDS):
    ...
    run_ = wandb.init(
        ...
        config=hparams,
        group = "gaf_11class_tpu_{}".format(EXPERIMENT_VERSION),
        name="gaf_11class_tpu_{}_{}".format(EXPERIMENT_VERSION, fold + 1),
    )
