...
def process_training_data(data_file):
    ...
    return {'inp1' : img, 'inp2' : tabular_data}, {"target2" : target2 } 
...
def process_testing_data(data_file):
    ...
    return {'inp1' : img, 'inp2' : tabular_data}, {"target2" : target2, "image_id":  data['image_id']} 
...
def multimodeleffbx():
    ...
    model = tf.keras.models.Model(inputs = [inp1, inp2], outputs = [output2])
...
for fold in range(6, yapl.config.FOLDS):
    ...
    run_ = wandb.init(
        ...
        name="2_class_{}_{}".format(EXPERIMENT_VERSION, fold + 1),
    )
    ...
    pd.DataFrame({
            ...
            'actual'  : np.argmax(test_labels["target2"].numpy(), axis=1), 
