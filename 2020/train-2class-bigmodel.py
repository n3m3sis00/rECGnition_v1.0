...
hparams = {
    ...
    "channels" : 1,
    'is fine tune' : "no",
    "Description" : "efficientnet Backbone use"
}
...
class configSetup(Config):
    def __init__(self):
        ...
        self.GCS_DS_PATH = KaggleDatasets().get_gcs_path('2class64')
        ...
        self.VAL_FILES = tf.io.gfile.glob(self.GCS_DS_PATH + '/val*.tfrec')
        self.TRAIN_CSV = self.GCS_DS_PATH + '/train.csv'
        self.VAL_CSV = self.GCS_DS_PATH + '/val.csv'
        self.TEST_CSV = self.GCS_DS_PATH + '/test.csv'
...
if tpu:
    ...
    yapl.config.BATCH_SIZE = 4 * strategy.num_replicas_in_sync 
else:
...
def multimodeleffbx():
    ...
    x     = tf.keras.layers.Dense(2, activation='softmax')(x1)
...
#                 validation_steps  =   yapl.config.TOTAL_VAL_IMG//yapl.config.BATCH_SIZE,
                # class_weight      =   hparams['class_weights'],
                verbose           =   1
            )
...
for fold in range(yapl.config.FOLDS):
    ...
    yapl.config.CALLBACKS[0] = tf.keras.callbacks.ModelCheckpoint(
                                    'fold-%i.h5'%(fold+1), monitor='val_accuracy', verbose=1, save_best_only=True,
