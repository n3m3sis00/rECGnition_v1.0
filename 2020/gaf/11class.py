...
hparams = {
    ...
    "channels" : 1,
    "Description" : "Gaf Trained for batch size 128",
    "folds" : 9
}
...
class configSetup(Config):
    def __init__(self):
        ...
        if args.train_on == "kaggle":
            ...
            self.GCS_DS_PATH = KaggleDatasets().get_gcs_path('mitbih-images-gaf-128x128')
            ...
            self.COUNT_FILE = '/kaggle/input/mitbih-images-gaf-128x128/image_count.json'
        elif args.train_on == "colab":
            self.GCS_DS_PATH = "/content/drive/MyDrive/Research papers/arrhythmia/CODE/input/gaf/mitbih-images-gaf-128x128"
            self.TRAIN_FILES = sorted(tf.io.gfile.glob(self.GCS_DS_PATH + '/data*.tfrec'))[:18]
            self.TEST_FILES = sorted(tf.io.gfile.glob(self.GCS_DS_PATH + '/data*.tfrec'))[18:]
            print(self.GCS_DS_PATH + '/image_count.json')
            self.COUNT_FILE = self.GCS_DS_PATH + '/image_count.json'
        else:
            from kaggle_datasets import KaggleDatasets
            self.GCS_DS_PATH = KaggleDatasets().get_gcs_path('mitbih-images-gaf-128x128')
            self.TRAIN_FILES = sorted(tf.io.gfile.glob(self.GCS_DS_PATH + '/data*.tfrec'))[:18]
...
with open(yapl.config.COUNT_FILE) as f:
...
for fold in range(1, yapl.config.FOLDS):
    ...
    run_ = wandb.init(
        ...
        config=hparams,
        group = "gaf_11class_GPU_{}".format(EXPERIMENT_VERSION),
        name="gaf_11class_GPU_{}_{}".format(EXPERIMENT_VERSION, fold + 1),
    )
