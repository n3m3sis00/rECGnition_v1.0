...
class configSetup(Config):
    def __init__(self):
        ...
        self.GCS_DS_PATH = KaggleDatasets().get_gcs_path('10class-images')
...
def process_training_data(data_file):
    ...
    label = tf.one_hot(data['target'], depth=11)
...
for fold in range(yapl.config.FOLDS):
    ...
    run_ = wandb.init(
        ...
        config=hparams,
        group = "11_class_{}".format(EXPERIMENT_VERSION),
        name="11_class_{}_{}".format(EXPERIMENT_VERSION, fold),
    )
