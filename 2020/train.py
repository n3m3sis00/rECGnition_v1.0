...
for fold in range(yapl.config.FOLDS):
    ...
    STEPS = (0.4 * yapl.config.TOTAL_TRAIN_IMG)//(yapl.config.BATCH_SIZE*4) + 1
    ...
    # pred = pred.squeeze()
    test_labels = next(iter(img_labels_ds.batch(int(0.4 * yapl.config.TOTAL_TRAIN_IMG)))).numpy()
    print(pred.shape)
