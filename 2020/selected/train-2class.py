...
hparams = {
    ...
    "channels" : 1,
    'is fine tune' : "no",
    "Description" : "this is the same experiment as 2_class_15_hack just GAF added colored"
}
...
class configSetup(Config):
    def __init__(self):
        ...
        self.GCS_DS_PATH = KaggleDatasets().get_gcs_path('2class64gaf')
        self.TRAIN_FILES = tf.io.gfile.glob(self.GCS_DS_PATH + '/train*.tfrec')
        self.TEST_FILES = tf.io.gfile.glob(self.GCS_DS_PATH + '/train*.tfrec')
        self.VAL_FILES = tf.io.gfile.glob(self.GCS_DS_PATH + '/train*.tfrec')
        self.TRAIN_CSV = self.GCS_DS_PATH + '/train.csv'
        self.VAL_CSV = self.GCS_DS_PATH + '/train.csv'
        self.TEST_CSV = self.GCS_DS_PATH + '/train.csv'
...
#configuration
yapl.config.TOTAL_TRAIN_IMG = len(pd.read_csv(yapl.config.TRAIN_CSV))
yapl.config.TOTAL_VAL_IMG = len(pd.read_csv(yapl.config.VAL_CSV))
yapl.config.TOTAL_TEST_IMG = len(pd.read_csv(yapl.config.TEST_CSV))
...
def process_training_data(data_file):
    ...
    label = tf.one_hot(data['target'], depth=2)
...
def multimodeleffbx():
    inp1  = tf.keras.layers.Input(shape = (*yapl.config.IMG_TRAIN_SHAPE, 3), name='inp1')
    inp2  = tf.keras.layers.Input(shape = (2,), name='inp2')

    x1 = tf.keras.layers.Conv2D(32, 3, activation='relu')(inp1)
...
print('##'*30)
print("#### IMAGE SIZE {} ".format((64, 64, 3)))
...
for fold in range(yapl.config.FOLDS):
    ...
    run_ = wandb.init(
        ...
        config=hparams,
        group = "2_class_{}".format(EXPERIMENT_VERSION),
        name="2_class_{}_{}".format(EXPERIMENT_VERSION, fold),
    )
    
    
    yapl.config.CALLBACKS[0] = tf.keras.callbacks.ModelCheckpoint(
                                    'fold-%i.h5'%(fold+1), monitor='val_loss', verbose=1, save_best_only=True,
    ...
    # pred = pred.squeeze()
    test_labels = next(iter(img_labels_ds.batch(int(yapl.config.TOTAL_TEST_IMG)))).numpy()
    print(pred.shape)
